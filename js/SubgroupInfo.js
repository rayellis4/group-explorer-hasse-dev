...
function showSubgroupLattice ( type /*: VisualizerType */ ) {
   ...
   function caption (H /*: subgroup */) {
      ...
      if (Group.representation[0][0] === '(') {
         elements = H.members
            .toArray()
            .map((el) => '<span style="white-space: nowrap">' + Group.representation[el] + '</span>')
            .join('<br>')
      } else {
         ...
         for (let inx = 0; inx * elementsPerLine < H.order; inx++) {
            ...
            elements += '<span style="white-space: nowrap">' + hRepresentations.slice(start, end).join(', ') + '</span><br>'
         }
      }
      const tooltipCss = [
         'position: relative',
      ...
      const tooltip = `<div class="toggled hidden" style="${tooltipCss}">${generators}<br>Elements:<br>${elements}</div>`
      const script = (subgroupIndex === 0) ?
           `<script>
              function displaySubgroup(event) {
                const $tooltip = $(event.target).closest('div').find('.toggled')
                const $textElement = $(event.target).closest('.TextElement')
                if ($tooltip.css('display') === 'none') {
                  $textElement.css('z-index', parseInt($textElement.css('z-index')) * 10000)
                } else {
                  $textElement.css('z-index', parseInt($textElement.css('z-index')) / 10000)
                }
                $tooltip.toggle()
            ...
            ''
      const text = `<div onclick="displaySubgroup(event)">${heading}${tooltip}${script}</div>`
      return text
   }
   
   // Build a sheet with subgroups shown at those locations.
   var sheetElementsAsJSON = [ ];
   Group.subgroups.map((H /*: Subgroup */, subgroupIndex /*: integer */) => {
      ...
      const sheetElement = (type === 'Hasse') ?
         { className: 'TextElement',
           ...
           color: '#d8d8d8',
           alignment: 'center',
           fontSize: cellHeight/6 + 'px',
           x: pos.x + hMargin,
           y: pos.y + vMargin,
           w:  cellHeight/3 - 2 * hMargin,
         } :
      ...
      sheetElementsAsJSON.push(sheetElement)
   })
